...
class MathToolsTestCase(unittest.TestCase):
    def test_basic_functions(self) -> None:
        ...
        self.assertAlmostEqual(
            MathTools.epley_1rm(100, 5, factor=1.1),
            100 * (1 + 0.0333 * 5) * 1.1,
        )
        with self.assertRaises(ValueError):
    ...
    def test_session_and_misc(self) -> None:
        self.assertAlmostEqual(MathTools.session_density(1000.0, 600), 100.0)
        self.assertAlmostEqual(MathTools.set_pace(30, 900), 2.0)
    ...
    def test_prescription_and_pyramid(self) -> None:
        ...
        enhanced = ExercisePrescription.exercise_prescription(
            weights,
            reps,
            timestamps,
            rpe,
        ...
        repo_t._delete_all("pyramid_tests")
        repo_e._delete_all("pyramid_entries")
        self.assertNotEqual(enhanced["analysis"]["current_1RM"], base_rm)
        ...
        trend = ExercisePrescription._analyze_1rm_trends(timestamps, weights)
        self.assertEqual(trend["trend"], "linear")
        plateau = ExercisePrescription._pyramid_plateau_detection(timestamps, weights)
        ...
        ts, rms, wts, mets = ExercisePrescription._process_pyramid_tests()
        self.assertEqual(len(ts), 2)
        self.assertIn("strength_reserve", mets[0])
        fatigue = ExercisePrescription._pyramid_enhanced_fatigue(ts, rms, wts, mets, 1000.0, ts[-1])
        alpha = ExercisePrescription._pyramid_enhanced_alpha(0.05, ts, rms, wts, mets, ts[-1])
        repo_t._delete_all("pyramid_tests")
        repo_e._delete_all("pyramid_entries")
        ...
        self.assertTrue(ExercisePrescription._validate_pyramid_test(valid_test))
        invalid_test = {
            "starting_weight": 100.0,
